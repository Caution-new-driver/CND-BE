...
> "✅ 구현됨"만 실제로 만들어서 테스트까지 끝낸 것이고, 나머지는 ERD·기능명세서 v4 기준으로 제가 제안한 형태입니다 — 실제 담당자가 다르게 만들고 싶으면 얼마든지 바뀔 수 있습니다.

공통 규칙(제안): 내부 운영 화면용 API는 `/api/**`, 고객 공개 화면용 API는 `/api/public/**`로 구분.
...
| b15 | POST | `/api/drops/{dropId}/publish` | 공개(PUBLISHED) 상태 전환 | 제안 |

---

## Stage 6 — 고객 구매 흐름 (담당: 이수현)

| ID | Method | Path | 설명 | 상태 |
| --- | --- | --- | --- | --- |
| b16 | GET | `/api/public/drops` | 공개 Drop 목록 | 제안 |
| b16 | GET | `/api/public/drops/{dropId}` | Drop 상세 + 신청 현황(N/M) | 제안 |
| b17+b18 | POST | `/api/public/drops/{dropId}/preorders` | 프리오더 신청 (1건=1개 고정, 조건부 UPDATE로 동시성 제어) | 제안 |

**열린 질문**:
- `Preorder.status` 값 목록 미정 (지금 코드엔 자유 문자열로 남겨둠)
- f10 분기 B "동시성 마감 실패" 케이스의 정확한 응답 형식이 기획서 자체에 미결정으로 남아있음 — `409 Conflict` + 에러 메시지 정도를 제안
...
3. 인증 없음이 확정이라 dropId 등 리소스 ID 자체가 사실상 접근 권한 역할을 하는데, 이대로 괜찮은지 (v4 스코프상 문제없다고 보지만 한 번 팀 확인 필요)
